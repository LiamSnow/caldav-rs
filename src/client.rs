...
impl CalDAVClient {
    ...
    async fn request(
        ...
        let full_url = if url.contains("http") {
            url.to_string()
        } else {
            add_path(&go_back(&self.home), url)
        };
    ...
    pub(crate) async fn calquery(&self, url: &str, depth: i32, body: &str) -> anyhow::Result<Element> {
        ...
        // println!("X method: {}, url: {}, depth: {}", method, url, depth);
        // println!("x body: {}", body);
        let res = self.request(method, url, depth, body).await.context("CALENDAR-QUERY request failed")?;
        Ok(res.parse().context(anyhow!("Parsing CALENDAR-QUERY failed. res: {}", res))?)
    }
